...
async function fetchCollectionProducts(collectionGid: string): Promise<any[]> {
  ...
  while (hasNextPage) {
    ...
    console.log(
      `[Collection Update] Fetched ${allProducts.length} products so far...`,
    );
  }
...
export async function POST(req: NextRequest) {
  try {
    ...
    console.log("[Collection Webhook] Updated - ID:", collectionData.id);
    ...
    // 2. Tüm güncel ürünleri sayfalama ile Shopify'dan çek
    ...
    console.log(
      `[Collection Webhook] Collection ${collectionData.id} updated with ${products.length} products`,
    );
